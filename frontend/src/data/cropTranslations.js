export const CROP_TRANSLATIONS = {
  rice: {
    en: "Rice / Paddy",
    hi: "चावल / धान",
    pa: "ਚੌਲ / ਝੋਨਾ",
    hindi_translit: "Dhan",
    punjabi_translit: "Jhona",
    category: "Cereal / Grain",
    growing_season: "Kharif (Jun - Nov)",
    image: "https://images.unsplash.com/photo-1586201375761-83865001e31c?auto=format&fit=crop&w=600&q=80"
  },
  maize: {
    en: "Maize / Corn",
    hi: "मक्का / भुट्टा",
    pa: "ਮੱਕੀ",
    category: "Cereal / Grain",
    growing_season: "Kharif / Rabi",
    image: "https://images.unsplash.com/photo-1551754655-cd27e38d2076?auto=format&fit=crop&w=600&q=80"
  },
  chickpea: {
    en: "Chickpea / Bengal Gram",
    hi: "चना",
    pa: "ਛੋਲੇ / ਚਨਾ",
    category: "Pulse / Legume",
    growing_season: "Rabi (Oct - Mar)",
    image: "https://images.unsplash.com/photo-1515543904379-3d757afe72e4?auto=format&fit=crop&w=600&q=80"
  },
  kidneybeans: {
    en: "Kidney Beans / Rajma",
    hi: "राजमा",
    pa: "ਰਾਜਮਾਂਹ",
    category: "Pulse / Legume",
    growing_season: "Kharif / Spring",
    image: "https://images.unsplash.com/photo-1588644525273-f37b60d78512?auto=format&fit=crop&w=600&q=80"
  },
  pigeonpeas: {
    en: "Pigeon Peas / Arhar (Tur)",
    hi: "अरहर / तूर दाल",
    pa: "ਅਰਹਰ / ਤੂਰ ਦਾਲ",
    category: "Pulse / Legume",
    growing_season: "Kharif (Jun - Dec)",
    image: "/assets/crops/pigeonpeas.jpg"
  },
  mothbeans: {
    en: "Moth Beans",
    hi: "मोठ दाल",
    pa: "ਮੋਠ ਦਾਲ",
    category: "Pulse / Legume",
    growing_season: "Kharif",
    image: "https://images.unsplash.com/photo-1607672632458-9eb56696346b?auto=format&fit=crop&w=600&q=80"
  },
  mungbean: {
    en: "Green Gram / Moong",
    hi: "मूंग दाल",
    pa: "ਮੂੰਗੀ ਦੀ ਦਾਲ",
    category: "Pulse / Legume",
    growing_season: "Kharif / Zaid",
    image: "/assets/crops/mungbean.png"
  },
  blackgram: {
    en: "Black Gram / Urad",
    hi: "उड़द दाल",
    pa: "ਮਾਂਹ ਦੀ ਦਾਲ (ਉੜਦ)",
    category: "Pulse / Legume",
    growing_season: "Kharif / Summer",
    image: "https://images.unsplash.com/photo-1546833999-b9f581a1996d?auto=format&fit=crop&w=600&q=80"
  },
  lentil: {
    en: "Lentil / Masoor",
    hi: "मसूर दाल",
    pa: "ਮਸਰਾਂ ਦੀ ਦਾਲ",
    category: "Pulse / Legume",
    growing_season: "Rabi (Oct - Apr)",
    image: "https://images.unsplash.com/photo-1599307767316-776533bb941c?auto=format&fit=crop&w=600&q=80"
  },
  pomegranate: {
    en: "Pomegranate",
    hi: "अनार",
    pa: "ਅਨਾਰ",
    category: "Horticulture / Fruit",
    growing_season: "Perennial",
    image: "https://images.unsplash.com/photo-1615485290382-441e4d049cb5?auto=format&fit=crop&w=600&q=80"
  },
  banana: {
    en: "Banana",
    hi: "केला",
    pa: "ਕੇਲਾ",
    category: "Horticulture / Fruit",
    growing_season: "Year Round",
    image: "https://images.unsplash.com/photo-1571771894821-ce9b6c11b08e?auto=format&fit=crop&w=600&q=80"
  },
  mango: {
    en: "Mango",
    hi: "आम",
    pa: "ਅੰਬ",
    category: "Horticulture / Fruit",
    growing_season: "Summer / Perennial",
    image: "https://images.unsplash.com/photo-1553279768-865429fa0078?auto=format&fit=crop&w=600&q=80"
  },
  grapes: {
    en: "Grapes",
    hi: "अंगूर",
    pa: "ਅੰਗੂਰ",
    category: "Horticulture / Fruit",
    growing_season: "Spring / Perennial",
    image: "https://images.unsplash.com/photo-1537640538966-79f369143f8f?auto=format&fit=crop&w=600&q=80"
  },
  watermelon: {
    en: "Watermelon",
    hi: "तरबूज",
    pa: "ਹਦਵਾਣਾ / ਤਰਬੂਜ਼",
    category: "Horticulture / Fruit",
    growing_season: "Zaid (Feb - May)",
    image: "https://images.unsplash.com/photo-1587049352846-4a222e784d38?auto=format&fit=crop&w=600&q=80"
  },
  muskmelon: {
    en: "Muskmelon",
    hi: "खरबूजा",
    pa: "ਖਰਬੂਜ਼ਾ",
    category: "Horticulture / Fruit",
    growing_season: "Zaid (Feb - May)",
    image: "https://images.unsplash.com/photo-1598170845058-32b9d6a5da37?auto=format&fit=crop&w=600&q=80"
  },
  apple: {
    en: "Apple",
    hi: "सेब",
    pa: "ਸੇਬ",
    category: "Horticulture / Fruit",
    growing_season: "Temperate / Perennial",
    image: "https://images.unsplash.com/photo-1560806887-1e4cd0b6cbd6?auto=format&fit=crop&w=600&q=80"
  },
  orange: {
    en: "Orange / Citrus",
    hi: "संतरा / किन्नू",
    pa: "ਸੰਤਰਾ / ਕਿੰਨੂ",
    category: "Horticulture / Fruit",
    growing_season: "Winter / Perennial",
    image: "https://images.unsplash.com/photo-1582979512210-99b6a53386f9?auto=format&fit=crop&w=600&q=80"
  },
  papaya: {
    en: "Papaya",
    hi: "पपीता",
    pa: "ਪਪੀਤਾ",
    category: "Horticulture / Fruit",
    growing_season: "Year Round",
    image: "https://images.unsplash.com/photo-1517282009859-f000ec3b26fe?auto=format&fit=crop&w=600&q=80"
  },
  coconut: {
    en: "Coconut",
    hi: "नारियल",
    pa: "ਨਾਰੀਅਲ",
    category: "Plantation / Fruit",
    growing_season: "Coastal / Perennial",
    image: "https://images.unsplash.com/photo-1584589167171-541ce45f1eea?auto=format&fit=crop&w=600&q=80"
  },
  cotton: {
    en: "Cotton",
    hi: "कपास (नरमा)",
    pa: "ਕਪਾਹ / ਨਰਮਾ",
    category: "Cash / Commercial",
    growing_season: "Kharif (Apr - Nov)",
    image: "https://images.unsplash.com/photo-1606041008023-472dfb5e530f?auto=format&fit=crop&w=600&q=80"
  },
  jute: {
    en: "Jute / Pat",
    hi: "पटसन / जूट",
    pa: "ਪਟਸਨ / ਜੂਟ",
    category: "Fiber Crop",
    growing_season: "Kharif (Mar - Jul)",
    image: "https://images.unsplash.com/photo-1598880940371-c756e015fea1?auto=format&fit=crop&w=600&q=80"
  },
  coffee: {
    en: "Coffee",
    hi: "कॉफ़ी",
    pa: "ਕੌਫੀ",
    category: "Plantation / Cash",
    growing_season: "Highland / Perennial",
    image: "https://images.unsplash.com/photo-1514432324607-a09d9b4aefdd?auto=format&fit=crop&w=600&q=80"
  }
};

export const getLocalizedCropName = (cropKey, lang = 'en') => {
  if (!cropKey) return 'Crop';
  const crop = CROP_TRANSLATIONS[cropKey.toLowerCase()];
  if (crop && crop[lang]) {
    return crop[lang];
  }
  return cropKey.charAt(0).toUpperCase() + cropKey.slice(1);
};
